...
import { getCurrentPatientId, insertRow, uploadImageToBucket } from "@/Services/Services";
  ...
  const uploadRecord = async () => {
    if (selectedImage) {
      ...
      try {
        ...
        // 5. Prepare document data using medical_event id and only fields that exist in documents table
        const documentData = {
          medical_event_id: data.id,
          patient_id: currentPatientId,
          file_url,
          file_size: null, // You can calculate this if needed
          is_processed: true, // Since we processed it through the API
        };
